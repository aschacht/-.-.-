...
public class GenerateAST {
	...
	public static void main(String[] args) throws IOException {
		...
		List<String> exprDefinition = Arrays.asList(
				...
				"Setat			: Expr callee , Expr.Literal index , Expr toset ", // (setat)
				"Sub			: Expr callee , Expr.Literal start , Expr.Literal end ", // (sub)
				"Binary			: Expr left , Token operator , Expr right", // logical - yroot
				"Mono			: Expr value , Token operator", // sin -tanh ln exp
	...
	private static void defineType(PrintWriter writer, String baseName, String className, String fields) {
		...
		for (String field : individualFields) {
			...
			if (split.length == 3) {
				name = field.split(" ")[2];
				writer.println("if (this." + name + "!=null)");
				writer.println("	str +=this." + name + ".toString()+\" \";");
			} else if (split.length == 2) {
				name = field.split(" ")[1];
				type = field.split(" ")[0];
				if (type.equals("Token")) {
					writer.println("if (this." + name + "!=null)");
					writer.println("	str+=this." + name + ".lexeme+\" \";");

				} else if (type.equals("boolean")) {

				} else if (type.equals("char")) {
					writer.println("	str+=this." + name + "+\" \";");
		...
		if(baseName.equals("Stmt")) {
			...
			"public boolean equals(Object obj) {\n"+
				"if(obj instanceof StmtDecl) {\n"+
					...
					+"return stmt instanceof "+className+" && this.expression.equals((("+className+")stmt).expression);\n"+
				"}else\n"
		...
		if (!className.equals("StmtDecl") && !className.equals("FunDecl")) {
			writer.println("	@Override");
